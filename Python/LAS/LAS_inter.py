...
def determine_min_max_hic():
    ...
    for pair in chr_pairs:
        ...
        if (os.path.isfile(fname) == True):
            # read in hic matrix
            hic_filename =  config["HIC_FILT_DIR"] +'hic_chr' + str(chr1) + '_chr' + str(chr2) + '_zscore.txt'
...
def draw_identified_LASregions(pair, minl, maxl, cell_type):
    chr1, chr2 = pair
    ...
    if (os.path.isfile(fname) == True):
        ...
        if cell_type == "IMR90":
            df = df.replace(np.min(df, axis = 0), -5)
...
def main():
    ...
    chr_pairs = list(itertools.combinations(config["chrs"], 2))
    Parallel(n_jobs = config['NUM_PROC'])(delayed(run_LAS)(pair, config["threshold"]) for pair in chr_pairs)  

    minl, maxl = determine_min_max_hic()
    print(minl, maxl)
    Parallel(n_jobs = config['NUM_PROC'])(delayed(draw_identified_LASregions)(pair, minl, maxl, config["CELL_TYPE"]) for pair in chr_pairs)  
