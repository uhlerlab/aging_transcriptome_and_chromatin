# Script adapted from https://github.com/anastasiyabel/functional_chromosome_interactions/blob/master/code/large_average_submatrix_hic_avgcutoff_iter.py
import sys
import numpy as np
...
def large_average_submatrix_adj(data, chr0, threshold_new):
    # store some data on iterations
    dir = config["HIC_NEW_DIR"] + str(chr0) + '_' + str(chr0) + '/'
...
def search_main(data, dir, iter):
    ...
    print("best_k: ", best_k, ", best_l:", best_l)
...
def get_hic_matrix(hic_filename, chr0):
    ...
    chr0_size = get_chrom_size(chr0, config["GENOME_DIR"])/config["HIC_RESOLN"] + 1
    
    hic_matrix = csr_matrix((contact_values, (row_ind, col_ind)), shape = (chr0_size, chr0_size))
    hic_dense = np.asarray(hic_matrix.todense())
    row_labels = np.arange(chr0_size)*config["HIC_RESOLN"]
    col_labels = np.arange(chr0_size)*config["HIC_RESOLN"]
...
def numclust_avg(chr0):
    
    fname = config["HIC_NEW_DIR"]  + 'intermingling_regions.chr' + str(chr0) + '_chr' + str(chr0) + '.csv'
    ...
    if (os.path.isfile(fname) == True):
        ...
        plt.xlabel('Cluster #')
        plt.ylabel('Score')
        plt.savefig(config["HIC_NEW_DIR"] + 'cluster_score.chr' + str(chr0) + '_chr' + str(chr0) + '.png')
        plt.close()
        ...
        plt.xlabel('Cluster #')
        plt.ylabel('Average')
        plt.savefig(config["HIC_NEW_DIR"] + 'cluster_average.chr' + str(chr0) + '_chr' + str(chr0) + '.png')
        plt.close()
...
def determine_min_max_hic():
    ...
    for chr0 in chr_list:
        fname = config["HIC_NEW_DIR"]  + 'intermingling_regions.chr' + str(chr0) + '_chr' + str(chr0) + '.avg_filt.csv'
        if (os.path.isfile(fname) == True):
            # read in hic matrix
            hic_filename =  config["HIC_FILT_DIR"] +'hic_chr' + str(chr0) + '_chr' + str(chr0) + '_zscore.txt'
            df = pd.read_csv(hic_filename, index_col = 0)
            data = df.to_numpy()
            min_chr = np.min(data)
            max_chr = np.max(data)
            min_list.append(min_chr)
            max_list.append(max_chr)
    minl = min(min_list)
    maxl = max(max_list)
    return minl, maxl

def draw_identified_LASregions(chr0, minl, maxl, cell_type):
    ...
    if (os.path.isfile(fname) == True):
        ...
        plt.imshow(data, cmap = 'Reds', vmin = -1, vmax = 5) #fix values to have same scale for both celltypes
        cbar = plt.colorbar()
        ...
        plt.savefig(config["HIC_NEW_DIR"] + 'hic_transformed_rescaled.chr' + str(chr0) + '_chr' + str(chr0) + 'commonscale.png')
...
def run_LAS(chr0, threshold_new):
    fname = config["HIC_NEW_DIR"] + 'intermingling_regions.chr' + str(chr0) + '_chr' + str(chr0) +'.avg_filt.csv'
    if (os.path.isfile(fname) == False):
        ...
        # run LAS algorithm
        start_rows, stop_rows, start_cols, stop_cols, best_score_list, avg_list = large_average_submatrix_adj(data, chr0, threshold_new)

        # convert indices to positions
        start_row_pos, stop_row_pos, start_col_pos, stop_col_pos = map_num2pos(df, start_rows, stop_rows, start_cols, stop_cols)

        # store results in dataframe
        dic = {'start row' : pd.Series(start_row_pos), 'stop row' : pd.Series(stop_row_pos), 
               'start col' : pd.Series(start_col_pos), 'stop col': pd.Series(stop_col_pos), 
               'score' : pd.Series(best_score_list), 'avg' : pd.Series(avg_list)}
...
def main():
    ...
    iters = 100
    print("LAS z-score threshold = ", config["threshold"])

    chr_list = config["chrs"]
    Parallel(n_jobs = config['NUM_PROC'])(delayed(run_LAS)(chr0, config["threshold"]) for chr0 in chr_list)  

    minl, maxl = determine_min_max_hic()
    print(minl, maxl)
    Parallel(n_jobs = config['NUM_PROC'])(delayed(draw_identified_LASregions)(chr0, minl, maxl, config["CELL_TYPE"]) for chr0 in chr_list)  
