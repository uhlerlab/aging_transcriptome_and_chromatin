...
def z_transform(hic_long):
    '''
    Normalizes the hic values to mean zero and variance one
    Args:
    ...
    '''
    hic_mean = np.mean(hic_long['value'])
    hic_std = np.std(hic_long['value'])
    
    # z-transformation
    hic_long['value_norm'] = (hic_long['value'] - hic_median) / hic_std
...
def plot_hist(df1, df2, binwidth, celltype):
    ...
    plt.hist(df1['value'], 
             ...
             alpha=0.5, label="MEFISTO")
    plt.hist(df2['value'], 
             bins=np.arange(min(df2['value']), max(df2['value']) + binwidth, binwidth), 
             alpha=0.5, label="random")
    plt.xlabel("Differences IMR90 - old_fibroblasts ranked Hi-C values")
    plt.ylabel("Counts")
...
def main():
    ...
    hic = pd.concat([inter_hic, intra_hic])
    
    # Subsetting 
    if subset == 'MEFISTO':
        hic_subset = subset_MEFISTO(hic, mefisto_dir, 0.1)
    else:
        # create random subset of size 350
        selected_genes = hic['chr1'].sample(350, random_state=1)
        hic_subset = hic[hic['chr1'].isin(selected_genes) & 
                           hic['chr2'].isin(selected_genes)]
    
    # Ranking of the values
    hic_subset['value_norm'] = hic_subset['value_norm'].rank() 
    
    # Convert from long to wide format
    hic_wide = long_to_wide(hic_subset)   
    
    # if all zero column exists, remove column and row corresponding to that locus
    hic_wide = hic_wide.loc[:, (hic_wide != np.min(hic_subset['value_norm'])).any(axis=0)]
    hic_wide = hic_wide.T.loc[:, (hic_wide.T != np.min(hic_subset['value_norm'])).any(axis=0)].T
