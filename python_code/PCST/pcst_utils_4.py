...
def get_prizes_combi(tr0, tr1, data_dir, save_dir, interactome, p_thr, cell_type, step_list):
    """ Gets prizes for differentially expressed genes (t -> t+1) and highly expressed TFs (at time t)
    ...
    # Add prizes to TFs from the "next older" network
    next_step = [step for step in step_list if step.startswith(tr1)]
    if len(next_step) == 0: #oldest network
        prizes = pd.concat([de_genes_0, de_genes_1])
    else:       
        net = pickle.load(open(save_dir + "network_"+cell_type+"_"+p_thr+"_step_"+next_step[0]+".pickle", "rb"))
        ...
        nodes = nodes.loc[(nodes['protein'].isin(TFs)) & (nodes['transition'] == 'tr0'), 'protein'].tolist()
        nodes = [node + '_tr1' for node in nodes]
        TF_prizes = pd.DataFrame({'name': nodes, 'prize': np.mean(de_genes_1['prize'])})

        # Combine all prized genes
        prizes = pd.concat([de_genes_0, de_genes_1, TF_prizes])
...
def get_PPI(step, data_dir, save_dir, cell_type, p_thr, step_list):
    ...
    prized_nodes = get_prizes_combi(tr0, tr1, data_dir, save_dir, interactome, p_thr, cell_type, step_list)
    tf_targets = tf_targets[tf_targets['protein2'].isin(prized_nodes['name'])]                                                 
...
def run_pcst_combi(step, graph_params, data_dir, save_dir, cell_type, p_thr, step_list):
    ...
    STRING, tf_targets = get_PPI(step, data_dir, save_dir, cell_type, p_thr, step_list)
    ...
    # Get prizes
    prizes_data = get_prizes_combi(tr0, tr1, data_dir, save_dir, STRING, p_thr, cell_type, step_list)
...
def compare_networks(net_dict, data_dir, fig_dir, save_dir, step_list, TFs_with_targets):
    ...
    fig, axs = plt.subplots(nrows=2, ncols=int(np.ceil(len(net_dict) / 2)), figsize=(10, 7))
    ...
    for (name, net), ax in zip(net_dict.items(), axs.ravel()):       
        ...
        STRING, tf_targets_subset = get_PPI(step, data_dir, save_dir, cell_type, p_thr, step_list)
        ...
        prizes_data = pd.read_csv(save_dir+'de_terminals_'+step+'.tsv', sep='\t')
...
def get_net_dir_all_stages(save_dir, cell_type, p_thr):
    ...
    """
    steps = ["1-9_10-29_30-59", "10-29_30-59_60-79", "30-59_60-79_80-89", "60-79_80-89_90-96"]
    net_dir = {}
...
def main():
    ...
    for step in step_list:
        ...
        augmented_forest = run_pcst_combi(step, graph_params, data_dir, save_dir, cell_type, p_thr, step_list)
