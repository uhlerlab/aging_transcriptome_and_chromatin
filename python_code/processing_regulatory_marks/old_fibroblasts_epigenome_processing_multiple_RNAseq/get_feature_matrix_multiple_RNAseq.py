...
def get_feature_matrix_chrom(bed_chrom, df, epigenome_dir, transcript_annotations, rna_seq_counts, rna_seq_counts2, rna_seq_counts3):
    '''
    ...
    colnames = feature_matrix.columns.values
    RNA_per_locus = get_rna_per_locus(bed_chrom, transcript_annotations, rna_seq_counts)
    RNA_per_locus.columns = colnames
    
    RNA_per_locus2 = get_rna_per_locus(bed_chrom, transcript_annotations, rna_seq_counts2)
    RNA_per_locus2.index = ["RNA-seq2"]
    RNA_per_locus2.columns = colnames
    
    RNA_per_locus3 = get_rna_per_locus(bed_chrom, transcript_annotations, rna_seq_counts3)
    RNA_per_locus3.index = ["RNA-seq3"]
    RNA_per_locus3.columns = colnames
    
    # Add RNA-seq counts to the feature matrix
    feature_matrix = pd.concat([feature_matrix, RNA_per_locus, RNA_per_locus2, RNA_per_locus3])
    return(feature_matrix)


def get_rna_per_locus(bed_chrom, transcript_annotations, rna_seq_counts):
    '''
    ...
    Returns:
        Pandas DataFrame with the summed RNA-seq count per locus
    '''    
...
def main():
    ...
    rna_seq_filename2 = config['RNA-SEQ_FILENAME_2']
    rna_seq_filename3 = config['RNA-SEQ_FILENAME_3']
    ...
    rna_seq_counts2 = get_rna_seq_counts(epigenome_dir, rna_seq_filename2)
    rna_seq_counts3 = get_rna_seq_counts(epigenome_dir, rna_seq_filename3)
    ...
    for chrom in chr_list:
        ...
        feature_matrix = get_feature_matrix_chrom(bed_chrom, df, epigenome_dir, transcript_annotations, rna_seq_counts, rna_seq_counts2, rna_seq_counts3)
