...
def get_diff_map_non_DE(diff_map, transition, data_dir, save_dir, seed):
    """ Creates diff map with non DE genes with same numbers of genes per chromosome as in a transition
    ...
    # get number of DE genes per chromosome in the transition of interest
    DE_genes_sub = DE_genes[DE_genes['transition'] == transition]
    DE_genes_sub = DE_genes_sub[DE_genes_sub['updown'] == updown]
    DE_genes_sub = DE_genes_sub[DE_genes_sub['locus'].isin(diff_map.columns)]
    chromosome_list = [locus.split("_")[1] for locus in DE_genes_sub['locus']] 
