...
def draw_identified_LASregions(pair, minl, maxl, cell_type):
    ...
    if (os.path.isfile(fname) == True):
        ...
        plt.imshow(data, cmap = 'Reds', vmin = -5, vmax = 16) #fix values to have same scale for both celltypes
        cbar = plt.colorbar()
...
def main():
    ...
    iters = 100
    #threshold_new = abs(scipy.stats.norm.ppf(config["pvalue_threshold"]))
    threshold_new = 35
