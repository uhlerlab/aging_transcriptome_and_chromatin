...
''' This script runs n simulations and outputs the number of differences between young and old fibroblasts'''
...
def main():
    ...
    for n_sim in tqdm(range(100)):
        ...
        # Select random loci
        selected_loci = all_gene_loci.sample(58, random_state=2022+n_sim).sort_index()['locus'].unique()
        
        # Binarize map for young and old fibroblasts
        hic_IMR90 = get_binarized_hic(selected_loci, chr_list, hic_dir, 'IMR90', resol)
        hic_old = get_binarized_hic(selected_loci, chr_list, hic_dir, 'old_fibroblasts', resol)

        # Calculate differences between young and old
        diff = hic_IMR90['value'] - hic_old['value']
        row = pd.DataFrame({'-1': [(diff == -1).sum()], 
                            '0': [(diff == 0).sum()], 
                            '1': [(diff == 1).sum()]})
        results = pd.concat([results, row], ignore_index = True)
    
    results.to_csv(hic_dir + 'simulation_differences_random_loci.csv')
