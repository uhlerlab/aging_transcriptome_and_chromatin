...
def get_submatrix_scores(sample, hic_r1, hic_r2, hic_dir, threshold, chr0, resol):
    '''
    Calculates the submatrix scores for both replicates
    Args:
        sample: (string) sample name that is used for identified submatrices
        hic_r1: (pd DataFrame) normalized HiC values for chr0 for replicate 1
        hic_r2: (pd DataFrame) normalized HiC values for chr0 for replicate 2
    ...
    fname = hic_dir + sample + '/LAS_intra-'+ str(threshold) + '/intermingling_regions.chr' + str(chr0) + '_chr' + str(chr0) + '.avg_filt.csv'
...
def main():
    ...
    for chr0 in tqdm(chr_list):
        ...
        fname1 = hic_dir + sample_r1 + '/final_BP' + str(resol) + '_intraKR_interINTERKR/hic_chr' + str(chr0) + '_chr' + str(chr0) + '_zscore.txt'
        fname2 = hic_dir + sample_r2 + '/final_BP' + str(resol) + '_intraKR_interINTERKR/hic_chr' + str(chr0) + '_chr' + str(chr0) + '_zscore.txt'
        ...
        for sample in ['Young_B1R1', 'Young_B1R2', 'Old_B1R1', 'Old_B2R2']:
            ...
            scores_sample = get_submatrix_scores(sample, hic_r1, hic_r2, hic_dir, threshold, chr0, resol)
        ...
        LAS_dir = save_dir + sample_r1.split("_")[0] + '_combi/LAS_intra-'+ str(threshold) 
        fname = LAS_dir  + '/intermingling_regions.chr' + str(chr0) + '_chr' + str(chr0) +'.avg_filt.csv'
