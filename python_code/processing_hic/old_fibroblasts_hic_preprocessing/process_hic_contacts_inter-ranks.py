...
def get_centromere_locations(genome_dir):
    ...
    '''
    centrom_filename = genome_dir+'hg19_centromeres'
...
def rank_based_inverse_normal_transformation(ranks, chr_pairs, processed_hic_data_dir):
    '''
    Rank-based inverse-normal-transformation of all Hi-C matrices for pairs of chromosomes in chr_pairs 
    Args:
        ranks: (pd DataFrame) df with all values and corresponding ranks
    ...
    for pair in tqdm(chr_pairs):
        ...
        df.columns = ['chr1', 'chr2', 'value']
        
        # replace HiC values by ranks
        df = df.merge(ranks, how = "left", on = "value")
        
        # Z-transformation: convert rank distribution to standard normal distribution
        n = np.max(df['rank'])
        df['rank'] = ss.norm.ppf(df['rank'] / (n+1))
        
        df = df[['chr1', 'chr2', 'rank']]
        
        # convert back to wide matrix
        df = long_to_wide(df)
        
        # save new matrix
        df.to_csv(processed_hic_data_dir+'hic_'+'chr'+str(chr1)+'_'+'chr'+str(chr2)+'_rank_INT.txt')
...
def long_to_wide(df):
    ...
    df_wide = df.pivot_table(index='chr1', columns='chr2', values='rank', sort = False).fillna(0)
...
def main():
    ...
    # Ranking and inverse normal transformation
    print('Ranking based inverse normal transformation:')
    ranks = pd.DataFrame({'value': nonzero_entries,
                          'rank': ss.rankdata(nonzero_entries)})
    ranks.to_csv(processed_hic_data_dir+'ranks.txt')
    print(ranks.shape)
    ranks = ranks.append({'value': 0, 'rank': 1}, ignore_index = True).drop_duplicates()
    print(ranks.shape)
    
    rank_based_inverse_normal_transformation(ranks, chr_pairs, final_hic_data_dir)
