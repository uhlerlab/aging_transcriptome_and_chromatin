...
pcst_TFs = read.delim(paste0(file_path, 'Data/incl_TFs_design2.csv'), sep = ",") %>%
...
shared_TFs_loadings <- filter(pcst_TFs, TF %in% TFs) %>%
  arrange(desc(value)) %>%
  subset(select = c(TF, value))

head(shared_TFs_loadings)

ggplot(shared_TFs_loadings, aes(x = TF, y = value)) +
  geom_point() +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 90)) +  
  theme(text = element_text(size = 17)) + 
  ylab('median target loading')
```
