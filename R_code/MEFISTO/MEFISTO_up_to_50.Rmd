...
figdir = paste0(file_path, "Figures/up_to_50/")
...
## Convert to long format and remove all that are older than 50
```{r}
...
data_long %<>% mutate(age_group = plyr::round_any(age, 10, f = floor))

#plot age distribution over the intervals
samples_binned <- subset(data_long, select = c(id, age_group, sex, nationality)) %>%
  distinct()
ggplot(samples_binned, aes(x = age_group, group = sex, fill = sex)) +
  geom_bar() +
  theme_bw() + 
  xlab('age intervals') +
  ylab('number of individuals') +
...
data_mean <- group_by(data_long, ensembl_id, age_group) %>%
...
df_pca <- ungroup(data_wide)%>%
  ...
  t() %>%
  data.frame()%>%
  mutate(age_group = colnames(data_wide[-1])) %>%
  mutate(age_group = factor(age_group, levels = age_group, order = TRUE))
...
```{r}
outfile = (paste0(file_path, "models/MEFISTO_up_to_50.hdf5"))
...
# Comparison to MEFISTO model on all ages
```{r}
model_all_ages <- load_model(paste0(file_path, "models/MEFISTO_3f.hdf5"))

# function to get loadings of the first factor
get_loadings_f1 <- function(model, ages){
...
loadings <- bind_rows(get_loadings_f1(model_all_ages, 'all_ages'), 
                      get_loadings_f1(trained_model, 'up_to_50')) %>%
  spread(key = name, value = value)

cor(loadings$all_ages, loadings$up_to_50, use="complete.obs")
```

```{r model_corr}
ggplot(loadings, aes(x = all_ages, y = up_to_50)) + 
  geom_point(size = 0.5) + 
  xlab('loadings factor 1 for 0-96 years model') +
  ylab('loadings factor 1 for 0-59 years model') +
  theme_bw() +
...
ggplot(df_plot, aes(x = age_group, y = mean, col = age_group)) + 
    geom_point() + 
    geom_line() +
...
# Save as csv
write.csv(top_genes_pos, paste0(file_path, "Data/top_genes_pos_0.1_50.csv"), row.names = FALSE)
write.csv(top_genes_neg, paste0(file_path, "Data/top_genes_neg_0.1_50.csv"), row.names = FALSE)
```
...
df = data_long %>%
  left_join(subset(gene_annotations, select = c(ensembl_id, symbol))) %>%
  filter(ensembl_id %in% selected_genes$ensembl_id) %>%
  subset(select = c(id, age, symbol, value)) %>%
  mutate(value = log(value + 1)) %>%
  spread(key = symbol, value = value)

model <- lm(age ~ ., subset(df, select = -id))
summary(model)
```
...
```{r}
df = data_long %>%
  subset(select = c(id, age, ensembl_id, value)) %>%
  mutate(value = log(value + 1)) %>%
  filter(ensembl_id %in% filtered_data$ensembl_id) %>%
  spread(key = ensembl_id, value = value)

model <- glmnet(subset(df, select = -c(id, age)), df$age, alpha = 1, lambda = 0.01)
...
```{r}
#length(which(lasso$symbol %in% top_genes$symbol)) / (nrow(df)-1)
venn.diagram(list(pos_loadings = top_genes_pos$symbol, pos_lasso = filter(lasso, coefficient > 0)$symbol), filename = "venn_pos.png")
venn.diagram(list(neg_loadings = top_genes_neg$symbol, neg_lasso = filter(lasso, coefficient > 0)$symbol), filename = "venn_neg.png")
```


```{r upset_lasso}
list = list(MEFISTO_down = top_genes_pos$symbol, lasso_pos = filter(lasso, coefficient > 0)$symbol,
            MEFISTO_up = top_genes_neg$symbol, lasso_neg = filter(lasso, coefficient < 0)$symbol)
