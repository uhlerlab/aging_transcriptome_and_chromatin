...
```{r venn_pos}
ggvenn(list(pos_loadings = top_genes_pos$symbol, pos_lasso = filter(lasso, coefficient > 0)$symbol)) + 
    theme(text = element_text(size = 15))
```


```{r venn_neg}
ggvenn(list(neg_loadings = top_genes_neg$symbol, neg_lasso = filter(lasso, coefficient < 0)$symbol)) + 
    theme(text = element_text(size = 15))
```

```{r upset_lasso}
list = list(MEFISTO_pos = top_genes_pos$symbol, lasso_pos = filter(lasso, coefficient > 0)$symbol,
            MEFISTO_neg = top_genes_neg$symbol, lasso_neg = filter(lasso, coefficient < 0)$symbol)
