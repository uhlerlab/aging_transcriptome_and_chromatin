...
# Query gene information from biomart
targets <- unique(TF_targets$target)
mart <- useEnsembl(biomart = "ensembl", 
                   dataset = "hsapiens_gene_ensembl", 
                   mirror = "useast")
gene_annotations <- getBM(filters="external_gene_name", 
...
TF_targets %<>% left_join(data_loci)
...
TF_targets %<>%
  ...
  replace_na(list(old_activity = "old:inactive")) %>%
  mutate(development = factor(paste0(young_activity, "_", old_activity), 
        levels = c("young:active_old:active", "young:active_old:inactive", "young:inactive_old:active", "young:inactive_old:inactive"))) %>% drop_na()
